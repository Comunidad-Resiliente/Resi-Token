...
export const tasks = () => {
  task('add-project', 'Add project')
    ...
    .setAction(async ({name}, {ethers}) => {
      ...
      const response = await ResiRegistry.connect(admin).addProject(bytesProjectName)
  ...
  task('add-projects', 'Add project')
    .addParam('names', 'Project names')
    .setAction(async ({names}, {ethers}) => {
      const [admin]: SignerWithAddress[] = await ethers.getSigners()
      const ResiRegistry: ResiRegistry = await ethers.getContract('ResiRegistry')
      const bytesProjectNames = names.map((name: string) => {
        return ethers.utils.formatBytes32String(name)
      })
      const response = await ResiRegistry.connect(admin).addProjects(bytesProjectNames)
